...
export default async function NotionGuides({
  searchParams,
}: {
  searchParams?: Promise<{ team?: string }>;
}) {
  const params = searchParams ? await searchParams : {};
  const team = (params as { team?: string }).team;
  const filteredGuides = team
    ? guides.filter((g) => g.team === team)
    : guides;
  const teamLabel = team ? teamLabels[team] : null;
  const backHref = team ? `/${team}` : "/";
      ...
      <div className="relative z-10 flex flex-col min-h-screen">
        ...
        <section className="px-6 pt-20 pb-14 text-center">
          ...
          >
            ...
            {team ? `Back to ${teamLabel}` : "Back to Portal"}
          </Link>
          ...
          >
            ...
            >
              {teamLabel ? `${teamLabel} — Reference Docs` : "Reference Docs"}
            </span>
          </div>
          ...
          >
            {teamLabel
              ? `Best practice guides for the ${teamLabel} workflow. Open any guide directly in Notion.`
              : "Deep-dive reference docs for every part of the job. Open any guide directly in Notion."}
          </p>
        </section>
        ...
        <CompletionFooter
          ...
          step="guides"
          nextStep="presentations"
          nextStepLabel="Presentations"
        />
